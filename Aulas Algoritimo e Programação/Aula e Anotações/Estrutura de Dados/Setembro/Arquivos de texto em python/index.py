...
with open("jogador1.csv", "w", newline="", encoding="utf-8") as f1, \
     open("jogador2.csv", "w", newline="", encoding="utf-8") as f2:
    
    jogador1 = csv.writer(f1)
    jogador2 = csv.writer(f2)

    # jogador1.writerow(["Nome", "ATK", "DEF", "SPD", "HP"])
    # jogador2.writerow(["Nome", "ATK", "DEF", "SPD", "HP"])
...
def Batalha():
    ...
    while (Fila or Pilha) and (Fila2 or Pilha2):
        ...
        print("\n=== Novo Duelo ===")
        with open("jogador1.csv", "a", newline="", encoding="utf-8") as f1, \
            open("jogador2.csv", "a", newline="", encoding="utf-8") as f2:
    
            jogador1 = csv.writer(f1)
            jogador2 = csv.writer(f2)

            jogador1.writerow(["\n=== Novo Duelo ==="])
            jogador1.writerow(["Jogador:", carta1])
            jogador1.writerow(["=================="])

            jogador2.writerow(["\n=== Novo Duelo ==="])
            jogador2.writerow(["Inimigo:", carta2])
            jogador2.writerow(["=================="])
        ...
        while carta1.Lif > 0 and carta2.Lif > 0:
            if carta1.Spd >= carta2.Spd:
                ...
                with open("jogador1.csv", "a", newline="", encoding="utf-8") as f1, \
                    ...
                    jogador1.writerow([f"{carta1.Name} atacou causando {dano} de dano! (HP inimigo: {carta2.Lif})"])
                    jogador2.writerow([f"{carta1.Name} atacou causando {dano} de dano! (HP inimigo: {carta2.Lif})"])
            ...
            #     for c in Fila:
            #         jogador1.writerow([c.Name, c.Atk, c.Def, c.Spd, c.Lif])
            #     for c in Fila2:
            #         jogador2.writerow([c.Name, c.Atk, c.Def, c.Spd, c.Lif])
            #     print("------------------")
